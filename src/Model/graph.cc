...
void s21::GraphModel::InitNeuronNet(int hide_layers,
...
void s21::GraphModel::NeuronsWeights(int index) {
...
int s21::GraphModel::GetMaxIndex() {
  int index = 0;
  double max = 0;
...
void s21::GraphModel::LoadInputLayer(std::deque<double> &input_layer) {
...
void s21::GraphModel::Sigmoida(int index) {
  for (size_t i = 0; i < neurons_[index].size(); ++i) {
    neurons_[index][i] = Sigmoid(neurons_[index][i]);
  }
}

double s21::GraphModel::Sigmoid(double x) { return 1.0 / (1.0 + exp(-x)); }

void s21::GraphModel::Softmax(int index) {
  double exp_sum = 0;
  for (size_t i = 0; i < neurons_[index].size(); ++i) {
...
Metrics s21::GraphModel::Testing(std::string filename, double test_value) {
...
void s21::GraphModel::Normalise(std::deque<double> &dataset) {
  for (size_t i = 0; i < dataset.size(); ++i) {
    dataset[i] /= 255;
  }
}

bool s21::GraphModel::IsEmptyGraph() { return neurons_.empty(); }

std::string s21::GraphModel::GetInfoGraph() {
  std::stringstream ss;
  ss << ("Перцептрон: ");
...
void s21::GraphModel::LoadWeightsFromFile(std::string path) {
