...
void s21::Model::InitNeuronNet(std::vector<int> size) {
  // ClearNeuronNet();
  if (neurons_.empty()) {
    for (auto v : size) {
      s21::Matrix layer;
...
void s21::Model::InitNeuronNet(std::vector<std::deque<double>>& dataset,
                               int in_size) {
  ClearNeuronNet();
  s21::Matrix input_layer(1, in_size);
...
void s21::Model::PrintInputLayer(std::deque<double>& values, int width) {
  for (int j = 0; j < 28; ++j) {
    for (int i = 0; i < 28; ++i) {
...
void s21::Model::PrintInputLayer(s21::Matrix& values, int width) {
  for (int j = 0; j < 28; ++j) {
    for (int i = 0; i < 28; ++i) {
      std::cout << std::setw(width);
...
void s21::Model::PrintSymbol(int index) {
  std::string name =
...
Metrics s21::Model::Testing(std::string filename, double test_value) {
  ...
  for (int i = 0; i < tests_amount; ++i) {
    if (dataset[i].size() == 785) {
      ...
      s21::Matrix& out_layer = neurons_[neurons_.size() - 1];
...
void s21::Model::SetValueInput(std::deque<double>& values) {
  neurons_[0].SetValue(values);
}

// обучение нейронной сети
void s21::Model::Learning(std::string filename, std::vector<int> layers,
...
void s21::Model::Forward(bool normalize) {
...
void s21::Model::Backward(int target_index, double alfa) {
  int n = weights_.size();  // количество матриц весов меньше на 1 кол-ва
                            // векторов нейронов
  s21::Matrix Y(1, neurons_[n].GetCols());  // полный вектор правильного ответа
  Y.SetValue(0);
  Y(0, target_index) = 1;

  s21::Matrix dE_dT = neurons_[n];
  dE_dT -= Y;  // dE_dT = Z - Y для выходного слоя
  // пройти и обновить все матрицы весов
  for (int i = n - 1; i >= 0; i--) {
    s21::Matrix& X = neurons_[i];
    s21::Matrix& W = weights_[i];
    s21::Matrix& B = biases_[i];
    s21::Matrix T = X;
    T *= W;
    T += B;
    ...
    s21::Matrix dE_dW = X.Transpose();
    dE_dW *= dE_dT;
    s21::Matrix dE_dB = dE_dT;
    // подготовить входные данные dE_dX(H) = dE_dT * Wt для следующего слоя
    s21::Matrix Wt = W.Transpose();
    dE_dT *= Wt;
...
void s21::Model::ClearNeuronNet() {
  neurons_.clear();
  weights_.clear();
  biases_.clear();
}
// запись информации в файл (метод пока не сохраняет значения беасов)!
void s21::Model::SaveWeightsToFile(std::string path) {
  std::ofstream out;
  out.open(path);
  if (out.is_open())
    for (int num = 0; num < 2; num++) {
      std::vector<s21::Matrix>& values = (num == 0 ? weights_ : biases_);
      for (auto m : values) {
        out << num << " ";
        out << m.GetRows() << " ";
        out << m.GetCols() << " ";
...
void s21::Model::LoadWeightsFromFile(std::string path) {
  std::vector<std::deque<double>> dataset;
  Parcer(dataset, path);
  ClearNeuronNet();
...
void s21::Model::Sigmoid(s21::Matrix& m) {
  for (int i = 0; i < m.GetRows(); ++i)
    for (int j = 0; j < m.GetCols(); ++j) m(i, j) = Sigmoid(m(i, j));
}
// функция активации
double s21::Model::Sigmoid(double x) { return 1.0 / (1.0 + exp(-x)); }
// производная функции Сигмоида
void s21::Model::Sigmoid_Deriv(s21::Matrix& m) {
  for (int i = 0; i < m.GetRows(); ++i)
...
// функция активации
void s21::Model::Softmax(s21::Matrix& m) {
  double sum = 0;
  for (int i = 0; i < m.GetRows(); ++i)
...
double s21::Model::Cross_Entropy(int index) {
...
std::pair<double, double> s21::Model::GetCoef(double x1, double x2, double y1,
                                              double y2) {
...
void s21::Model::Shuffle(std::vector<std::deque<double>>& dataset) {
...
// парсер
void s21::Model::Parcer(std::vector<std::deque<double>>& dataset,
                        std::string filename) {
...
Metrics s21::Model::Metrics_Calc(Precision p) {
  Metrics m;
...
void s21::Model::Normalize(s21::Matrix& m, double d1, double d2) {
...
// нормализация входных данных
void s21::Model::Normalize(s21::Matrix& m) {
  for (int i = 0; i < m.GetRows(); ++i)
